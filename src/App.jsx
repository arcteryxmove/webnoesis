...
const Progress = ({ value, tone = "emerald" }) => {
  const colors = tone === "emerald" ? "from-emerald-400 to-emerald-600" : "from-zinc-200 to-zinc-400";
  return (
...
function Library({ lessons, filter, setFilter, onStart, isCompleted, onOpen }) {
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {filtered.map(l => (
          <Card key={l.id}>
            ...
            <div className="flex items-center gap-2">
              <Button onClick={() => onOpen(l)}>{isCompleted(l.id) ? "Открыть урок" : "Начать"}</Button>
              {isCompleted(l.id) ? (<div className="text-emerald-400 text-sm">Завершено</div>) : (<Button variant="ghost" onClick={() => onStart(l)}>Отметить изученным</Button>)}
...
function Quests({ quizzes, sequential, onSubmit, onSubmitSequence, profile, selected }) {
...
function SequentialStep({ step, locked, onSubmit, result }) {
  ...
  return (
    <div className={`p-3 rounded-xl border ${locked ? 'border-zinc-900 text-zinc-500' : 'border-zinc-800'}`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <Button variant="ghost" onClick={()=>!locked && setOpen(o=>!o)}>{locked ? 'Закрыто' : (open ? 'Скрыть' : 'Пройти')}</Button>
        </div>
      </div>
      {(open && !locked) && (
        <div className="mt-3 space-y-3">
          {step.questions.map((q, idx) => (
            <div key={idx} className="bg-zinc-950 border border-zinc-900 rounded-xl p-3">
              <div className="text-sm mb-2">{q.q}</div>
              <div className="grid gap-2">
                {q.options.map((opt, i) => (
                  <label key={i} className={`flex items-center gap-2 p-2 rounded-xl border ${answers[idx]===i?"border-emerald-500 bg-emerald-500/10":"border-zinc-800 bg-zinc-900"}`}>
        ...
        <Card>
          ...
          <div className="space-y-2">
            {achievements.map((a,i)=>(
              <div key={i} className={`flex items-center justify-between rounded-xl border px-3 py-2 ${a.earned ? 'border-emerald-500 bg-emerald-500/10' : 'border-zinc-800 bg-zinc-950'}`}>
                <div>
...
function Leaderboard({ users, selectUser, selected }) {
...
function Admin({ libraryExtra, setLibraryExtra, quizzesExtra, setQuizzesExtra, users, addUser, removeUser, updateUserPoints }) {
    ...
    <Page>
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <div className="grid gap-2 text-sm">
            ...
            <input className="bg-zinc-900 border border-zinc-800 rounded-xl p-2" placeholder="kind (article / video / game / podcast / quiz)" value={item.kind} onChange={e=>setItem({...item, kind:e.target.value})}/>
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-6">
        ...
        <Card>
          ...
          <div className="grid gap-2 text-sm">
            <input className="bg-zinc-900 border border-zinc-800 rounded-xl p-2" placeholder="id (уникальный)" value={newUser.id} onChange={e=>setNewUser({...newUser, id:e.target.value})}/>
            <input className="bg-zinc-900 border border-zinc-800 rounded-xl p-2" placeholder="Имя" value={newUser.name} onChange={e=>setNewUser({...newUser, name:e.target.value})}/>
            <input type="number" className="bg-zinc-900 border border-zinc-800 rounded-xl p-2" placeholder="Очки" value={newUser.points} onChange={e=>setNewUser({...newUser, points:Number(e.target.value||0)})}/>
            <input className="bg-zinc-900 border border-zinc-800 rounded-xl p-2" placeholder="Статус" value={newUser.status} onChange={e=>setNewUser({...newUser, status:e.target.value})}/>
            <input className="bg-zinc-900 border border-zinc-800 rounded-xl p-2" placeholder="Роль" value={newUser.role} onChange={e=>setNewUser({...newUser, role:e.target.value})}/>
            <div className="flex gap-2 mt-2"><Button onClick={()=>{ addUser(newUser); setNewUser({ id: '', name: '', points: 0, status: 'Новичок', role: 'ученик' }); }}>Сохранить</Button></div>
          </div>
          <div className="mt-3 space-y-2 text-sm">
            {users.map(u => (
              <div key={u.id} className="flex items-center justify-between border border-zinc-800 rounded-xl p-2">
                ...
                <div className="flex items-center gap-2">
                  <input type="number" className="w-24 bg-zinc-900 border border-zinc-800 rounded-xl p-1 text-right" value={u.points} onChange={e=>updateUserPoints(u.id, Number(e.target.value||0))}/>
...
function LessonPage({ lesson, onClose, onComplete, completed }) {
  ...
  return (
    <Page>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          {!completed && <Button onClick={() => onComplete(lesson)}>Отметить как пройдено</Button>}
          <Button variant="ghost" onClick={onClose}>Закрыть</Button>
        </div>
      </div>
      <Card>
        <div className="aspect-video rounded-xl bg-zinc-900 border border-zinc-800 mb-4 flex items-center justify-center text-zinc-600">Плейсхолдер видео/вставки</div>
        {lesson.content ? (
...
function MemberProfile({ user, onClose }) {
  if (!user) return null;
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-20">
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 max-w-lg w-full relative">
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100">
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {m.tab === "library" && (
          <Library lessons={libraryAll} filter={m.filter} setFilter={m.setFilter} onStart={(l)=>{ m.completeLesson(l); alert("Урок отмечен как изученный. +20 очков и рост навыка."); }} isCompleted={(id)=>!!m.profile.completed[id]} onOpen={(lesson)=>{ setOpenLesson(lesson); m.setTab('lesson'); }} />
        )}
        {m.tab === "quests" && (<Quests quizzes={quizzesAll} sequential={SEQUENTIAL_QUESTS} onSubmit={m.submitQuiz} onSubmitSequence={(seqId, step, answers)=>m.submitSequence(seqId, step.id, step, answers)} profile={m.profile} selected={selectedQuiz} />)}
        ...
        {m.tab === "lesson" && (<LessonPage lesson={openLesson} onClose={()=>{ setOpenLesson(null); m.setTab('library'); }} onComplete={(l)=>{ m.completeLesson(l); }} completed={!!m.profile.completed[openLesson?.id]} />)}
        {m.tab === "leaderboard" && (<><Leaderboard users={m.users} selectUser={(u)=>{ setSelectedMember(u); m.setTab('leaderboard'); }} selected={selectedMember} />{selectedMember && <MemberProfile user={selectedMember} onClose={()=>setSelectedMember(null)} />}</>)}
        {m.tab === "admin" && (<Admin libraryExtra={m.libraryExtra} setLibraryExtra={m.setLibraryExtra} quizzesExtra={m.quizzesExtra} setQuizzesExtra={m.setQuizzesExtra} users={m.users} addUser={m.addUser} removeUser={m.removeUser} updateUserPoints={m.updateUserPoints} />)}
